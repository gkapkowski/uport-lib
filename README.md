...
**Uport** is a system for self-sovereign digital identity. This is the client side library that is used to interact with the mobile application where the end-user's keys are stored.
...
Integrating **uport** into your DApp is simple. You only need to use the `web3` provider given by the `uport-lib` instead of the regular `web3` library.
