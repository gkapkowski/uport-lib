...
class RemoteMutablePersona extends Persona {
   ...
   *  @return          {Object}         self
   */
  constructor(proxyAddress, ipfs, web3Provider, handleURI, msgServer, registryAddress) {
  ...
  writeToRegistry() {
    ...
    const obj = {
      add: {
        attributes: self.attrToAdd,
        claims: self.claimsToAdd
      },
      remove: {
        attributes: self.attrToRemove,
        claims: self.claimsToRemove
      }
    }
    return new Promise((accept, reject) => {
      // add to ipfs (correct call here)
      self.ipfs.add(obj, (err, ipfsHash) => {
        console.log(err, ipfsHash)
        if (err) {
          reject(err);
          return;
        }
        pushToMobile.call(self, ipfsHash)
      })
    })
  }
